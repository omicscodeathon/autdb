...
# -*- coding: utf-8 -*-
"""
Build Engagnition basic rows (facts only) and save to Excel ONLY.
...
3) Saves a single Excel file:
      <--out-xlsx>   (sheet: "Engagnition basic")
4) Prints a summary and the final line "Engagnition basic".
...
def main():
    ap = argparse.ArgumentParser(description="Build Engagnition basic table (Excel only).")
    ap.add_argument("--data-root", required=True, help="Path to dataset root (contains Engagnition/...).")
    ap.add_argument("--out-xlsx", required=True, help="Full path to save 'Engagnition basic.xlsx'.")
    args = ap.parse_args()
    ...
    # Save Excel ONLY (single sheet)
    xlsx_path = os.path.abspath(args.out_xlsx)
    try:
        with pd.ExcelWriter(xlsx_path, engine="xlsxwriter") as xlw:
            eng.to_excel(xlw, index=False, sheet_name="Engagnition basic")
        print(f"[OK] Saved Excel: {xlsx_path} (rows={len(eng)}, cols={len(eng.columns)})")
    except Exception as e:
        raise SystemExit(f"[ERR] Failed to write Excel: {xlsx_path}\n{e}")

    # Small summary
    print("[INFO] value counts: condition")
    if "condition" in eng.columns:
        print(eng["condition"].value_counts(dropna=False).to_string())
